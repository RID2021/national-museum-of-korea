...
- 검증: 자동 테스트 12개, 내장 프로필 8개의 실제 브라우저 로딩, 대화창 표시. 브라우저 검수는 로컬 위젯에서 수행했습니다.
- 맵 설정 완료 (2026-09-14): NPC 7개의 기존 오브젝트에 ZEP Script 상호작용을 추가하고, 배경에 포함된 로비 안내 로봇 앞에는 대화 영역을 배치했습니다. 해당 7개 맵에 `seoul2_ museum` 노말 앱도 연결·저장했습니다. 상세 값과 검증 한계는 [에디터 작업 기록](editor-setup-2026-09-14.md)을 확인하세요.
- 실제 플레이 확인: 사유의 방과 신라실(2)에서 관리자 채팅 명령으로 대화창이 열림을 확인했습니다. 순수비의 프로필·대사도 표시됩니다. 모든 NPC의 현장 F/영역 진입 실행은 아직 검증하지 못했습니다. 앱 시작 시 오류 알림이 관찰되었으며 상세 원인은 미확인입니다.
- 미구현: 글자 수집, 정답 입력·판정, 벽돌 배열, 금관 찾기, 지도 퍼즐, 관람 예절 컨베이어, 기억 카드 게임. `quiz`는 질문을 보여주는 대화이며 게임이 아닙니다.
...
`success`와 `ending`은 추후 게임 성공 판정에서 호출할 대본입니다. 현재는 검수용으로 독립 실행되며 미션 진행률·인벤토리·보상을 변경하지 않습니다. 공개 맵에 자유 접근 가능한 성공/엔딩 오브젝트를 배치하지 마세요.
