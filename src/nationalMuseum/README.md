...
### 최신 수정: 고분 진행률 UI 및 조건부 편의 이동

- 사진의 우측 임시 HUD를 제거했습니다. 기존 진행률 버튼에서 `seoul_2`와 동일한 `mission-progress-widget.html` 원형 게이지를 엽니다. 템플릿은 원본과 SHA-256까지 동일하며 박물관의 7개 미션 판정만 연결합니다.
- 진행률 화면 하단 버튼은 안내 대화 완료 기록 또는 선행 미션 완료 + 성공 대화 완료 조건에 따라 다음 장소로 이동합니다. 예: 호우총 미션과 성공 대화 완료 → 로비(2)에서 **백제실로 이동하기** 활성화.
- 진행 중인 대화/게임이나 남은 성공 대화가 있으면 이동을 차단합니다. 조건은 버튼 클릭 때 서버에서 다시 검사합니다.
- 성공 대화에 설정된 자동 이동은 유지합니다. 이동한 맵에서는 NPC를 직접 찾아가야 하며 입장 자동 대화는 실행하지 않습니다.
- 아래 탐험형 수정 기록의 우측 HUD 설명은 과거 상태입니다. 현재는 고분과 같은 진행률 창을 사용하고, NPC 대화는 현장 F/모바일 상호작용 버튼으로 실행합니다.

### 최신 수정: 탐험형 진행 및 모바일 입력

- 일반 맵 입장 시 NPC/게임 자동 실행을 제거했습니다. 단, 외부맵(밤)의 2쪽 도입 나레이션, 사유의 방의 관람 종료 방송, 백제실·신라실(1)의 최초 안내, 모든 임무를 마친 낮 맵의 엔딩은 각각 1회 자동 실행합니다.
- 우측 HUD는 **주변 NPC 대화**와 **임무 위치 안내**입니다. 대화는 NPC로부터 가로·세로 2타일 이내에서만 실행되고 안내 버튼은 이동/대화를 실행하지 않습니다. F/모바일 액션도 가까운 NPC와 상호작용합니다.
- 로비(5) 안내 로봇의 현장 좌표를 연결했습니다. 로비(4) 실제 신라실 문 `(98,45)`을 밟으면 `spawnAtMap`으로 이동합니다. 실제 도보 통과와 신라실 입장 후 자동 대화 없음까지 확인했습니다.
- 모바일 입력칸은 ZEP `showPrompt` 기본 입력창을 열어 키보드로 입력한 답을 돌려받고, 미션의 정답 확인 버튼으로 제출합니다. 글자 선택 우회 UI는 제거했습니다. 데스크톱은 직접 입력이며 입력 중 DOM을 교체하지 않습니다.
- 실제 휴대폰의 키보드 표시 성공은 사용자 확인 대기입니다. 자동 테스트 28개와 빌드/TypeScript 검사를 통과했습니다. 과거 MVP 검수의 자동 진입·원격 이동 설명은 아래 최신 기록으로 대체합니다: [탐험형 수정 기록](exploration-input-audit-2026-09-14.md).
...
닫은 게임은 해당 NPC를 다시 찾아가 대화하면 이어집니다. **처음부터 다시 → 초기화하고 시작**은 본인의 박물관 진행만 초기화하고 밤 입장 맵으로 돌아갑니다. 인벤토리와 다른 저장 데이터는 유지합니다.
